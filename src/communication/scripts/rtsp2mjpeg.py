...
# ---------------- FFmpeg helpers ----------------
def build_rtsp_url(host: str, path: str, username: str = "", password: str = "", port: int = 554) -> str:
    auth = ""
    if username:
        auth = f"{quote(username, safe='')}:{quote(password or '', safe='')}@"
    return f"rtsp://{auth}{host}:{port}/{path.lstrip('/')}"

def build_ffmpeg_cmd(rtsp_url: str, fps: int = 10, quality: int = 7, tcp: bool = True) -> List[str]:
    ...
    cmd = [
        ...
        "-loglevel", "error",
        "-rtsp_transport", "tcp",
        "-stimeout", "5000000",     # 5s I/O timeout (beberapa build mendukung opsi ini)
        "-i", rtsp_url,
        "-an",                      # tanpa audio
        "-sn",
        "-f", "mjpeg",
        "-q:v", "7",                # kualitas JPEG (1 terbaik, 31 paling rendah)
        "-r", "10",                 # target fps
        "-",
    ]
...
def jpeg_frames_from_pipe(proc: subprocess.Popen, stop_flag: threading.Event, buf_limit: int = 2_000_000):
    ...
    while not stop_flag.is_set():
        ...
        while True:
            s = buf.find(SOI)
            if s < 0:
                if len(buf) > buf_limit: buf.clear()
                break
            e = buf.find(EOI, s+2)
            if e < 0:
                if s > 0: del buf[:s]
                break
            frame = bytes(buf[s:e+2]); del buf[:e+2]
            yield frame
...
class StreamWorker:
    def __init__(self, node: Node, name: str, rtsp_url: str, topic: str,
                 hub: MjpegHub, hub_idx: int, fps: int, quality: int,
                 stale_timeout_sec: float, watchdog_period_sec: float):
        self.node = node; self.name = name; self.rtsp_url = rtsp_url
        self.topic = topic; self.hub = hub; self.hub_idx = hub_idx
        self.fps = fps; self.quality = quality
        self.stale_timeout = stale_timeout_sec; self.watchdog_period = watchdog_period_sec
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self._run_loop, name=f"{self.name}-reader", daemon=True)
        self.thread.start()
        self.wd_thread = threading.Thread(target=self._watchdog_loop, name=f"{self.name}-watchdog", daemon=True)
    ...
    def stop(self):
        ...
        if self.wd_thread: self.wd_thread.join(timeout=2.0)

    def _spawn_proc(self) -> subprocess.Popen:
        cmd = build_ffmpeg_cmd(self.rtsp_url, fps=self.fps, quality=self.quality, tcp=True)
        self.node.get_logger().info(f"[{self.name}] launching ffmpeg: {' '.join(cmd)}")
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=0, preexec_fn=os.setsid)
        # tail stderr
        self._stderr_thread = threading.Thread(target=self._read_stderr, args=(p,), daemon=True)
    ...
    def _run_loop(self):
        ...
        while not self.stop_flag.is_set():
            try:
                ...
                if rc is None:
                    self.node.get_logger().warn(f"[{self.name}] stdout EOF while ffmpeg still running (rc=None). Killing & restarting...")
                else:
                    self.node.get_logger().warn(f"[{self.name}] ffmpeg exited rc={rc}; restarting...")
                ...
                time.sleep(backoff); backoff = min(backoff*2.0, 10.0)
            finally:
                with self._lock:
                    if self.proc:
    ...
    def _watchdog_loop(self):
        while not self.stop_flag.is_set():
            now = time.time()
            idle = now - self._last_frame_time if self._last_frame_time else float("inf")
            if idle > self.stale_timeout:
                # 1) log
                self.node.get_logger().warn(f"[{self.name}] stale {idle:.1f}s > {self.stale_timeout}s → reset ffmpeg + drop MJPEG clients")
...
class Rtsp2MjpegNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("hosts", ["192.168.100.35","",""])
        self.declare_parameter("paths", ["Streaming/channels/101","Streaming/channels/101","Streaming/channels/102"])
        ...
        self.declare_parameter("topics", ["/cam1/image_raw/compressed","/cam2/image_raw/compressed","/cam3/image_raw/compressed"])
        ...
        hosts = list(self.get_parameter("hosts").get_parameter_value().string_array_value)
        paths = list(self.get_parameter("paths").get_parameter_value().string_array_value)
        ports = list(self.get_parameter("ports").get_parameter_value().integer_array_value)
        users = list(self.get_parameter("usernames").get_parameter_value().string_array_value)
        pwds  = list(self.get_parameter("passwords").get_parameter_value().string_array_value)
        fps = int(self.get_parameter("fps").value)
        quality = int(self.get_parameter("quality").value)

        urls = [build_rtsp_url(hosts[i], paths[i], users[i], pwds[i], ports[i]) for i in range(3)]
        topics = list(self.get_parameter("topics").get_parameter_value().string_array_value)
        topics = (topics + [""]*3)[:3]
        ...
        for i in range(3):
            if not hosts[i] or not paths[i]:
                ...
                continue
            w = StreamWorker(self, f"stream{i+1}", urls[i], topics[i],
                             hub=self.hub, hub_idx=i, fps=fps, quality=quality,
                             stale_timeout_sec=stale, watchdog_period_sec=wd)
    ...
    def _build_app(self) -> Flask:
        app = Flask(__name__)
        hub = self.hub; limit = self.mjpeg_limit
        ...
        @app.get("/cam2.mjpeg")
        def cam2(): return stream(1)
        @app.get("/cam3.mjpeg")
        def cam3(): return stream(2)
